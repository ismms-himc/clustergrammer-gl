...
export default (function find_mouseover_element(regl, params, ev) {
  ...
  _.each(["x", "y"], function (inst_axis) {
    ...
    offcenter[inst_axis] =
      (params.viz_dim.canvas[dim_dict[inst_axis]] *
        params.viz_dim.offcenter[inst_axis]) /
      2;
    // calculate relative to min position before zooming
    cursor_rel_min[inst_axis] =
      params.zoom_data[inst_axis].cursor_position -
      viz_dim_heat[inst_axis].min -
      offcenter[inst_axis];
    // reflect zooming and panning in relative to min calculation
    cursor_rel_min[inst_axis] =
      cursor_rel_min[inst_axis] / params.zoom_data[inst_axis].total_zoom -
      params.zoom_data[inst_axis].total_pan_min;
  ...
  if (params.tooltip.in_bounds_tooltip) {
    ...
    _.each(inst_dims, function (inst_axis) {
      if (inst_axis === "row") {
        ...
        axis_indices[inst_axis] =
          params.labels.ordered_labels[inst_axis + "_indices"][axis_index];
      } else {
      ...
      mouseover[inst_axis].name =
        params.labels.ordered_labels[inst_axis + "s"][axis_index];
      ...
      _.each(params.cat_data[inst_axis], function (d, cat_index) {
        inst_cat_name =
          params.labels.ordered_labels[inst_axis + "_cats-" + cat_index][
            axis_index
          ];
    ...
    if (params.tooltip.tooltip_type === "matrix-cell") {
      ...
      if ("mat_data_iz" in params) {
        params.int.mouseover.value_iz =
          params.mat_data_iz[axis_indices.row][axis_indices.col];
      }
    }
  }
