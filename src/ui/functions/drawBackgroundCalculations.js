...
export default (function draw_background_calculations(regl, params) {
  _.each(["row", "col"], function (inst_axis) {
    if (params.labels.queue.high[inst_axis].length > 0) {
      const inst_name = params.labels.queue.high[inst_axis][0];
      const inst_text_vect = vectorize_label(params, inst_axis, inst_name);
      params.visualization.text_triangles[inst_axis][inst_name] =
        inst_text_vect;
