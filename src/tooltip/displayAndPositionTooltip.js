...
export default (function display_and_position_tooltip(params) {
  ...
  if (params.tooltip.tooltip_type === "matrix-cell") {
    d3.select(params.tooltip_id)
      ...
      .style("margin-top", function () {
        const total_y_offset =
          params.zoom_data.y.cursor_position - d3_tip_height;
        return total_y_offset + "px";
      });
  } else if (params.tooltip.tooltip_type === "row-label") {
    d3.select(params.tooltip_id)
      .style("margin-left", function () {
        const total_x_offset = 150 + params.cat_data.row.length * 12;
        return total_x_offset + "px";
      })
      .style("margin-top", function () {
        const total_y_offset =
    ...
    d3.select(params.tooltip_id)
      ...
      .style("margin-top", function () {
        const total_y_offset = 125 + params.cat_data.col.length * 12;
        return total_y_offset + "px";
      });
