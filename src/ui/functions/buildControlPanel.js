...
import buildReclusterSection from "../../controlPanel/buildReclusterSection";
...
export default function build_control_panel(
  ...
  control_svg
    ...
    .on("click", function () {
      d3.selectAll(
        state.visualization.rootElementId + " .panel_button_titles"
      ).attr("opacity", 0.5);
      ...
      if (state.cat_viz.current_panel === "recluster") {
        ...
        d3.select(
          state.visualization.rootElementId + " .top_button_title"
        ).text("COL");
        d3.select(
          state.visualization.rootElementId + " .bottom_button_title"
        ).text("ROW");
        d3.selectAll(
          state.visualization.rootElementId + " .reorder_buttons"
        ).style("display", "block");
        d3.select(
          state.visualization.rootElementId + " .run_cluster_container"
        ).style("display", "none");
        d3.selectAll(
          state.visualization.rootElementId + " .dist_options"
        ).style("display", "none");
        d3.selectAll(
